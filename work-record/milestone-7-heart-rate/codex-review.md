...
The implemented `schemaVersion == 4` and 34 CSV columns match `work-record/milestone-3-export/spec.md`. No CSV ordering/count defect was found. The existing fixture does exercise the old `SetRecord.barWeightValue` and new Workout/AppPreferences defaults, but contains zero `ExerciseRestOverride` rows.
