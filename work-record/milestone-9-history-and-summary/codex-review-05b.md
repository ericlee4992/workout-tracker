...
The ticket requires the heart-rate series to be written by the finish path and says every way out of a workout takes coordinator `end` (work-record/milestone-9-history-and-summary/issues/05-finish-summary-heart-rate-graph.md:9-15, work-record/milestone-9-history-and-summary/issues/05-finish-summary-heart-rate-graph.md:43-46). Ordinary replacement now does. Template-drift replacement still finishes and saves before `startNew`, which then sees no active workout and cannot call `end` (WorkoutTracker/Features/Start/StartWorkoutView.swift:164-182, WorkoutTracker/Features/Start/StartWorkoutView.swift:208-217, WorkoutTracker/Domain/TemplateDrift.swift:193-205).
