...
1. **Digit-bearing brand refusal is only partial.** `work-record/scanner-accuracy/issues/02-brand-repair.md:18-30,174-175`; `WorkoutTracker/Domain/MachineLabelRepair.swift:159-183,208-231`; `WorkoutTrackerTests/MachineLabelRepairTests.swift:65-83`.
...
2. **Current source documentation contradicts the amended ticket and executable behavior.** `WorkoutTracker/Domain/MachineLabelRepair.swift:46-52,154-156,187-190,218-221`; `WorkoutTrackerTests/MachineLabelRepairTests.swift:35-42`; `work-record/scanner-accuracy/issues/02-brand-repair.md:25-29`.
...
3. **The ticket's canonical Resolution is stale and conflicts with its later response.** `work-record/scanner-accuracy/issues/02-brand-repair.md:90-98,141-146,167-184`.
