...
1. **The new provenance calculation adds repeated allocation to the existing main-actor live-ranking hot path.** `WorkoutTracker/Domain/CatalogMatcher.swift:227,398-401`; `work-record/scanner-accuracy/spec.md:9-12`.
...
3. **The rebuild deliberately drops explicit ticket behavior without amending the ticket.** `work-record/scanner-accuracy/issues/02-brand-repair.md:18-28,37-45,128-130,136-138`; `WorkoutTracker/Domain/MachineLabelRepair.swift:198-209,231-248`; `WorkoutTrackerTests/MachineLabelRepairTests.swift:35-45,65-76,106-123`.
...
5. **The new consecutive-line brand pass reintroduces the punctuation/spacing loss fixed for the single-line pass.** `WorkoutTracker/Domain/MachineLabelRepair.swift:82-105`; response at `work-record/scanner-accuracy/issues/02-brand-repair.md:141-147`.
...
6. **The repair commit contains unrelated next-ticket planning.** `work-record/scanner-accuracy/issues/03-capture-first.md:1-44`; `work-record/scanner-accuracy/spec.md:32-43`.
