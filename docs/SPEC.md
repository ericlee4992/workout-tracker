...
- Adding a machine can **photograph its name plate** (D33–D35): a viewfinder with a plate-shaped framing box and a one-tap shutter (scanner accuracy, ticket 03 — nothing is read until the shutter, and only inside the box); Vision reads the still on device, the reading is repaired for logo misreads (ticket 02), the app ranks catalog models against it and offers the candidates with scores, and the user confirms one — or creates a user-space model prefilled from the reading. The photo is discarded after reading; nothing leaves the phone by default. **Ask AI (D53, scanner-accuracy tickets 05–06):** with the developer's key saved in Settings, a plate the phone could not place offers one button that sends the plate inside the box — plus an 8 % margin around the box, and at most nine tenths of the photo — to Claude and ranks what it reads through the same matcher; on create-new, one button sends the manufacturer and model as typed, the plate's lines, and the app's own exercise list (every exercise's id, name and muscle group) so Claude can propose which exercises the machine serves, from that list only. Never automatic, never the whole frame, and the user confirms everything. A scan identifies the *model*, never which of the gym's three chest presses this is — that is the machine's label, which the user writes.
...
- **Templates are generic** exercise lists. Starting a workout at a gym resolves each exercise to the last-used machine there. A finished workout can be saved as a template from the finish sheet or, later, from its History detail (the menu's "Save as Template…", offered only when it has completed sets).
...
- No backend, accounts, or third-party runtime dependencies. **HealthKit and a watchOS companion are in scope from 2026-08-22 (D41)**, reopened deliberately for heart rate; everything else in this line still holds, and the app still talks to nothing but the device it is on — except the opt-in Ask AI taps (D53: the developer's own key now; a proxy is a prerequisite before any other user).
