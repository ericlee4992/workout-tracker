...
- No backend, accounts, or third-party runtime dependencies. **HealthKit and a watchOS companion are in scope from 2026-08-22 (D41)**, reopened deliberately for heart rate; everything else in this line still holds, and the app still talks to nothing but the device it is on — except consented OpenAI requests (D56: developer’s own key for the private trial; a proxy is a prerequisite before shared-key use by other users).
