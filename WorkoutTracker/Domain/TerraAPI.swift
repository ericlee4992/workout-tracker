...
struct TerraClient: Sendable {
    ...
    func complete(instructions: String, input: String, schema: [String: Any], name: String,
        ...
        do { (data, response) = try await session.data(for: request) }
            ...
            throw TerraError.message(error.code == .timedOut ? "OpenAI took too long. Try again." : "Could not reach OpenAI. Check your connection or continue manually.")
        }
        try Task.checkCancellation()
        guard let response = response as? HTTPURLResponse else { throw TerraError.invalidResponse }
        guard (200..<300).contains(response.statusCode) else {
            switch response.statusCode {
            case 401, 403: throw TerraError.message("Check your OpenAI key and model access in Settings.")
            case 429: throw TerraError.message("OpenAI usage is limited. Check API credits or try again later.")
            default: throw TerraError.message("OpenAI is unavailable (\(response.statusCode)). Try again later.")
            }
        }
