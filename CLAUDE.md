...
3. **`docs/DECISIONS.md`** — why it is that way (D1–D54, T1–T7). Record decision changes here; reopen a locked decision deliberately rather than drifting from it.
4. **`work-record/milestone-<n>-<name>/issues/`** — the work record: every ticket with its acceptance criteria and resolution notes, plus the Codex cross-reviews (`codex-review*.md`). Milestones 2, 3, 7, 8 and 9 have folders (`milestone-2-core-loop`, `milestone-3-export`, `milestone-7-heart-rate`, `milestone-8-history-and-charts`, `milestone-9-history-and-summary`), alongside single-feature folders.
...
- Swift 5 / SwiftUI, **iOS 26+** (D42; the deployment target is 26.0), iPhone-only, portrait. No third-party runtime dependencies.
...
Local markdown under `work-record/` — remote: github.com/ericlee4992/workout-tracker (private). See `docs/agents/issue-tracker.md`.
