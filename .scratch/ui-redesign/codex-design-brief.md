...
THE BRIEF is .scratch/ui-redesign/spec.md in this worktree — read it fully first. It is the
...
- SCREENSHOTS are the deliverable. Write a UI test class WorkoutTrackerUITests/CodexScreenshotUITests.swift
  ...
  codex-04-start (if you did Start). Then export the PNGs:
    xcodebuild test … -only-testing:WorkoutTrackerUITests/CodexScreenshotUITests \
      -resultBundlePath .scratch/ui-redesign/results/codex.xcresult
    xcrun xcresulttool export attachments --path .scratch/ui-redesign/results/codex.xcresult \
      --output-path .scratch/ui-redesign/screenshots/codex-raw
  and copy each PNG to .scratch/ui-redesign/screenshots/codex/<attachment-name>.png (use
  manifest.json in the raw folder for the names). Commit the PNGs.
- When done, write .scratch/ui-redesign/codex-design-report.md: your palette (hex values), the
