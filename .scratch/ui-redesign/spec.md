...
Effort folder `.scratch/ui-redesign/` (`spec.md`, `issues/01..09`, `screenshots/`). One branch per
...
xcodebuild test-without-building -project WorkoutTracker.xcodeproj -scheme WorkoutTracker \
  ...
  -resultBundlePath .scratch/ui-redesign/results/02.xcresult
xcrun xcresulttool export attachments --path .scratch/ui-redesign/results/02.xcresult \
  --output-path .scratch/ui-redesign/screenshots/02-raw     # then rename via manifest.json
```
